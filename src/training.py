# from https://github.com/graphcore/examples/blob/master/gnn/schnet/pytorch_geometric/molecular_property_prediction_with_schnet_code_only.py
...
#TODO this is basically the same as the above
torch.manual_seed(0)
knn_graph = KNNInteractionGraph(cutoff=cutoff, k=batch.num_nodes - 1)
model = SchNet(cutoff=cutoff, interaction_graph=knn_graph)
model = to_fixed_size(model, batch_size=2)
pop_model = poptorch.inferenceModel(model, options)
padded_ipu = pop_model(padded_batch.z, padded_batch.pos, padded_batch.batch)
...
torch.save(training_model, "trainedModel.model")
torch.save(training_model.model, "trainedModel_internal.model")
